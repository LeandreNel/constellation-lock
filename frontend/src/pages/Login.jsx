...
export default function Login() {
  ...
  function handleCanvasClick(e) {
    ...
    setSequence((prev) => {
      if (prev.includes(closest)) return prev.filter((id) => id !== closest);
      return [...prev, closest];
    });
  }
  ...
  async function handleSubmit() {
    ...
    if (sequence.length < 3) {
      setIsError(true);
      setMsg("Recreate your constellation key (at least 3 stars).");
      return;
    }
    ...
    try {
      ...
      setIsError(false);
      setMsg(`Welcome back, ${res.data.username}!`);
  ...
  return (
    <div className="page">
      ...
      <p>Sign in by recreating your constellation key</p>
      ...
      <div className="canvas-container">
        <p className="star-info">
          {sequence.length === 0
            ? "Recreate your constellation key"
            : `${sequence.length} star${sequence.length !== 1 ? "s" : ""} selected`}
        </p>
        <canvas
          ref={canvasRef}
      ...
      <p style={{ color: "#6a90c8", fontSize: "0.9rem" }}>
        ...
        <Link
          to="/register"
          style={{ color: "#b39dff", textDecoration: "none" }}
        >
          Register
        </Link>
      </p>
